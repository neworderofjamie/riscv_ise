...
def plot_var(stem, num_rows=4, num_cols=4, offset=0):
    ...
    for i, a in enumerate(var_axes.flatten()):
        a.plot(var_device[:,offset + i], alpha=0.5, label="Device")
        a.plot(var_sim[:,offset + i],  alpha=0.5, label="Simulation")
    a.legend()
