...
from pyfenn import (BackendFeNNHW, BackendFeNNSim, EventContainer, Model, 
                    PerformanceCounter, ProcessGroup, Runtime, Shape, Parameter)
from pyfenn.models import Linear, LinearWithSTDP, Memset, RNGInit
from models import LI, LIF, Bernoulli, Linear_LIF_STDP, BernoulliProbSpikeVar
...
# Group processes
neuron_update_processes = ProcessGroup([extra_input.process, primary_input.process, output.process], None,name="neuron_update_processes_group")
synapse_update_processes = ProcessGroup([extra_input_output.process, primary_input_output.process], None,name="synapse_update_processes_group")
...
for primary_prob_spike in probs_spike_primary:
    ...
    print("\n")
    print("Probability of spiking for main presynaptic neuron: " + str(primary_prob_spike))

    primary_prob_spike_formatted = np.round(primary_prob_spike * (1 << num_frac_bits_prob_spike)).astype(np.int16)
    copy_and_push(primary_prob_spike_formatted, primary_input.prob_spike, runtime)

    for post_weight in extra_spike_weight:

        filename = str(primary_prob_spike) + '_' + str(post_weight) + '.csv'

        print("Weight for presynaptic neuron: " + str(post_weight))

        # Load weights
        extra_input_weights = np.ones(32*10,dtype='int16') * np.round(post_weight* (1 << num_frac_bits)).astype(np.int16)
