                                   ...
                                   uint32_t startInstRetPtr, uint32_t endInstRetPtr, uint32_t startCyclePtr, uint32_t endCyclePtr, bool simulate)
{
    return AssemblerUtils::generateStandardKernel(
        ...
        [=](CodeGenerator &c, VectorRegisterAllocator &vectorRegisterAllocator, ScalarRegisterAllocator &scalarRegisterAllocator)
        {
            ...
            // alpha = e^(-1/20)
            // Since there are 13 fractional bits (of 20?), 
            // we divide the immediate value by 2^13 to
            // get the fraction 7792/8192=.95. Because 
            // there is nothing in the remaining 7 bits, we 
            // are left with 0+.95=.95
            c.vlui(*VAlpha, 7792);
            ...
            // v_thresh = 1
            // This is basically just 2^13/2^13=1
            ...
            c.L(loop);
            {
                ...
                // v += i
                // each of the 32 neurons in the vector receive a different input
                c.vadd(*VV, *VV, *VI);
    
                // spike = VV >= VThres
                // VTGE stands for vector greater than or equal
                c.vtge(*SSpike, *VV, *VVThresh);
    
                // v = spk ? v_reset : v
                // Think this corresponds to vector set equal (e.g., xto zero vector)
                c.vsel(*VV, *SSpike, *VVReset);

                // Store spikes in buffer
                // Buffer holds 32 bit integers because remember we only need
                // one 32 bit integer to keep track of which neurons in the 
                // register spiked
                c.sw(*SSpike, *SSpikeBuffer);
                c.addi(*SSpikeBuffer, *SSpikeBuffer, 4);
        
                //vmem[a...a+32] = v
                // store vector of voltages in buffer
...
{
    std::ofstream spikes("lif_with_fusi_spikes.csv");
...
void recordV(const int16_t *vRecordingData, uint32_t numTimesteps)
{
    ...
    std::ofstream voltages("lif_with_fusi_voltages.csv");
    for(uint32_t t = 0; t < numTimesteps; t++) {
...
int main(int argc, char** argv)
{
    ...
    CLI::App app{"LIF with Fusi STDP neuron simulation"};
    ...
    // Dump initial data to coe file; not sure if I need the two lines below
    ...
    if(shouldDisassemble) {
        std::cout<<"Here is the disassembled code!"<<std::endl;
